...
export const setupRoomUserService = () => {
  ...
  class RoomUserService extends BaseSchema {
    ...
    async setRole(ctx) {
      ...
      const data: any = await broker.call('roomUser.getOne', { userId, roomId });
      ...
      const res = await broker.call('roomUser.create', { userData: roleData });
      pubSub.publish('roomUserRoleChanged', roleData.role, { userId, roomId });
      return res;
    }
    ...
    async getBans(ctx) {
      ...
      return Promise.all(data.map(async (userRoom) => {
        const user = await broker.call('user.getOne', { userId: userRoom.userId });
        return {
          ...userRoom,
          user
        };
