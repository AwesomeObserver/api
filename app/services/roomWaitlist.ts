...
export const setupRoomWaitlistService = () => {
	...
	class RoomWaitlistService extends BaseSchema {
		...
		async get(ctx) {
			const { roomId } = ctx.params;
			let queue: any = await broker.call('roomWaitlist.getOne', {
				roomId
			});
		...
		async setPlay(ctx) {
			...
			if (!userId) {
				...
				if (!roomSource) {
					...
					if (waitlistQueue.users.length === 0) {
						...
						await broker.call('roomWaitlist.remove', {
							roomId,
							userId: nextUserId
						});
						return broker.call('roomWaitlist.setPlay', {
							roomId,
							userId: nextUserId
						});
					}
				...
				const [sourceData, userData]: any = await Promise.all([
					broker.call('roomUserPlaylist.cutFirst', {
						roomId,
						userId
					}),
			...
			broker.call('room.setContentTitle', {
				roomId,
		...
		async endPlay(ctx) {
			...
			if (waitlistQueue.users.length === 0) {
				await broker.call('roomWaitlist.setPlay', {
					roomId,
					userId: waitlistQueue.userId
				});
			} else {
				const nextUserId = parseInt(waitlistQueue.users[0], 10);
				await broker.call('roomWaitlist.remove', {
				...
				await broker.call('roomWaitlist.add', {
					roomId,
					userId: waitlistQueue.userId
				});
			}
